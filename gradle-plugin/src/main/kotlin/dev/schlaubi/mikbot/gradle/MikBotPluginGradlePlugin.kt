...
class MikBotPluginGradlePlugin : Plugin<Project> {
    ...
    private fun Project.createAssemblePluginTask(jarTask: Jar) = tasks.run {
        register<Jar>("assemblePlugin") {
            ...
            into("lib") {
                from({
                    val mainConfiguration = TransientDependencies.TRANSIENT_DEPENDENCIES
                        .lines()
                        .filterNot { it.startsWith("#") || it.isBlank() }

                    // filter out dupe dependencies
                    configurations.getByName("runtimeClasspath").files.filter {
    ...
    private fun Project.createPublishingTasks(assemblePluginTask: TaskProvider<Jar>) {
        tasks.apply {
            ...
            val copyFilesIntoRepo = task<Copy>("copyFilesIntoRepo") {
                group = "publishing"
                dependsOn(buildRepository)
            }

            afterEvaluate {
                copyFilesIntoRepo.apply {
//                    from(assemblePluginTask)
                    include("*.zip")
                    // providing version manually, as of weird evaluation errors
                    into(buildRepository.targetDirectory.get().resolve("${project.name}/$version"))
                }
            }
        }
    }
